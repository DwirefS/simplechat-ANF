...
def upload_to_blob(temp_file_path, user_id, document_id, blob_filename, update_callback, group_id=None, public_workspace_id=None):
    ...
    Automatically routes to Azure NetApp Files Object REST API when STORAGE_BACKEND='anf',
    otherwise uses Azure Blob Storage (default).

    Note: ANF Object REST API requires manual setup via Azure Portal.
...
def _upload_to_anf_internal(temp_file_path, user_id, document_id, blob_filename, update_callback, group_id=None, public_workspace_id=None):
    """Uploads the file to Azure NetApp Files Object REST API.
    ...
    - Object REST API (used here) for application access
    - NFS for Linux/Unix clients
    - SMB for Windows clients

    Prerequisites: Object REST API must be configured via Azure Portal.
    See: https://learn.microsoft.com/en-us/azure/azure-netapp-files/object-rest-api-access-configure
    """
    is_group = group_id is not None
    is_public_workspace = public_workspace_id is not None
    ...
    try:
        ...
        if not anf_client:
            raise Exception(
                "ANF storage client not available. Check ANF_OBJECT_API_ENDPOINT and credentials. "
                "Object REST API requires manual setup via Azure Portal."
            )
...
def upload_to_storage(temp_file_path, user_id, document_id, blob_filename, update_callback, group_id=None, public_workspace_id=None):
    ...
    This is an alias for upload_to_blob, which handles storage backend selection.
    ...
    Args:
        ...
        blob_filename: Target filename in storage
    ...
    """
    # upload_to_blob now handles storage backend selection internally
